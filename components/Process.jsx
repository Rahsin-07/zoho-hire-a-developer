      ...
      <div className="container">
        <div className="row align-items-center g-5">
          ...
          <div className="col-lg-7 fade-up" style={{ transitionDelay: '0.1s' }}>
            {steps.map((step, i) => (
              <div key={i}
                style={{ display: 'flex', gap: 28, alignItems: 'flex-start', padding: '28px 0', position: 'relative', transition: 'all 0.35s cubic-bezier(0.22,1,0.36,1)', cursor: 'default' }}
                onMouseEnter={(e) => {
                  e.currentTarget.style.transform = 'translateX(6px)'
                  const circle = e.currentTarget.querySelector('.step-num')
                  if (circle) {
                    circle.style.transform = 'rotate(-10deg) scale(1.08)'
                    circle.style.boxShadow = '0 10px 26px rgba(26,86,219,0.45)'
                  }
                }}
                onMouseLeave={(e) => {
                  ...
                  if (circle) {
                    ...
                    circle.style.boxShadow = '0 4px 16px rgba(26,86,219,0.3)'
                  }
                }}
              >
                {i < steps.length - 1 && (
                ...
                <div style={{ paddingTop: 12 }}>
                  <div style={{ fontSize: '1.1rem', fontWeight: 700, color: 'var(--dark)', marginBottom: 6, fontFamily: 'Inter, sans-serif' }}>{step.title}</div>
                  <div style={{ fontSize: '0.9rem', color: 'var(--muted)', lineHeight: 1.65, fontFamily: 'Inter, sans-serif' }}>{step.desc}</div>
                </div>
              </div>
            ))}
          </div>
