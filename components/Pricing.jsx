            ...
            <h3 className="col-md-6 col-lg-4 fade-up" key={plan.type} style={{ transitionDelay: `${i * 0.1}s` }}>
              <div
                style={{
                  ...
                  boxShadow: plan.featured ? '0 18px 40px rgba(15,23,42,0.18)' : 'none',
                }}
                onMouseEnter={e => {
                ...
                onMouseLeave={e => {
                  e.currentTarget.style.transform = plan.featured ? 'translateY(-6px)' : ''
                  e.currentTarget.style.boxShadow = plan.featured ? '0 18px 40px rgba(15,23,42,0.18)' : ''
                }}
              >
                {plan.popular && (
                  <div style={{
                ...
                <div style={{ fontFamily: 'Inter, sans-serif', fontSize: '2.8rem', fontWeight: 800, color: plan.featured ? '#fff' : '#0f172a', lineHeight: 1, marginBottom: 4, letterSpacing: '-1.5px' }}>
                ...
                <div style={{
                  fontSize: '0.82rem',
                  fontWeight: 600,
                  background: plan.featured ? 'rgba(255,255,255,0.1)' : 'linear-gradient(90deg,#3b82f6,#8b5cf6)',
                ...
                <ul className="list-unstyled text-start mb-4" style={{ fontSize: '0.875rem', color: plan.featured ? 'rgba(255,255,255,0.75)' : '#334155', fontFamily: 'Inter, sans-serif' }}>
                  {plan.features.map(f => (
                    <li key={f} className="mb-2" style={{ transition: 'transform 0.25s ease, padding-left 0.25s ease' }}
                      onMouseEnter={e => { e.currentTarget.style.transform = 'translateX(4px)' }}
                      onMouseLeave={e => { e.currentTarget.style.transform = 'translateX(0)' }}
                    >
                      <i className="bi bi-check-circle-fill me-2" style={{ color: '#10b981' }} />{f}
                    </li>
                  ))}
                </ul>
                {plan.featured
                  ? <a href="#consultation" style={{ background: '#0d6efd', color: '#fff', border: 'none', borderRadius: 10, padding: '0.85rem 1.5rem', fontFamily: 'Inter, sans-serif', fontWeight: 700, fontSize: '0.875rem', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, textDecoration: 'none', width: '100%', transition: 'all 0.3s cubic-bezier(0.22,1,0.36,1)' }}
                      onMouseEnter={e => { e.currentTarget.style.transform = 'translateY(-2px)'; e.currentTarget.style.boxShadow = '0 12px 28px rgba(13,110,253,0.4)' }}
                      onMouseLeave={e => { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.boxShadow = 'none' }}
                    >Hire Our Developer Now <i className="bi bi-arrow-right" /></a>
                  : <a href="#consultation" style={{ background: 'transparent', color: '#0f172a', border: '2px solid #e8e3dd', borderRadius: 10, padding: '0.82rem 1.5rem', fontFamily: 'Inter, sans-serif', fontWeight: 700, fontSize: '0.875rem', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, textDecoration: 'none', width: '100%', transition: 'all 0.3s cubic-bezier(0.22,1,0.36,1)' }}
